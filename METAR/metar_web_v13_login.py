...
def crear_componente_viento():
    """
    Crea una interfaz más amigable para el viento con campos separados
    """
    col1, col2, col3, col4 = st.columns([2, 2, 1, 3])
    
    with col1:
        dir_viento = st.text_input(
            "Dirección", 
            key='dir_viento',
            placeholder="360",
            help="Grados (0-360)"
    ...
    with col2:
        int_viento = st.text_input(
            "Intensidad", 
            key='int_viento', 
            placeholder="15",
            help="Nudos base (ej: 15, 15G25)"
        )
    ...
    with col4:
        ...
        with col4_1:
            var_desde = st.text_input(
                "Var Desde",
                key='var_desde',
                placeholder="340",
                help="Variación desde (grados)"
            )
        with col4_2:
            var_hasta = st.text_input(
                "Var Hasta",
                key='var_hasta',
                placeholder="080",
                help="Variación hasta (grados)"
            )
    
    if var_desde and var_hasta:
        variacion_completa = f"{var_desde}V{var_hasta}"
    else:
        variacion_completa = ""
    
    return dir_viento, int_viento, variacion_completa

def crear_componente_fenomenos():
    """
    Sistema de selección múltiple de fenómenos con botón +
    """
    ...
    texto_fenomenos = " ".join([f.split(" - ")[0] for f in st.session_state.fenomenos_seleccionados])
    return texto_fenomenos
...
def crear_componente_precipitacion():
    """
    Campo obligatorio de precipitación en formato PPTRZ
    """
    st.markdown("**PRECIPITACIÓN (OBLIGATORIO):**")
    st.caption("""
    **Formato PPTRZ:**
    - **PPTRZ** = Trazas (< 0.1 mm)
    ...
    if not pp_seleccionado:
        st.error("⚠️ DEBE seleccionar un valor de precipitación")
        return None
    
    st.success(f"✅ Precipitación: {pp_seleccionado} ({OPCIONES_PP[pp_seleccionado]})")
    return pp_seleccionado
...
def generar_metar(datos):
    try:
        ...
        registro = {
            ...
            'Temperatura': temp,
            'Punto_Rocío': rocio,
            'Humedad_Relativa_%': datos['hr'] if datos['hr'] else hr_calculada,
            'QNH': qnh,
...
with col_izq:
    with st.form(key='metar_form'):
        ...
        pp_valor = crear_componente_precipitacion()
        
        st.markdown("---")
        
        # TN/TX - Automático según hora
        ...
        if generar:
            ...
            if mostrar_tn_tx and not st.session_state.get('tn_tx_completo'):
