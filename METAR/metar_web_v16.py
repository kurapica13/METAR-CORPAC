...
def verificar_autenticacion():
    """Login 100% HTML - sin widgets Streamlit en la pantalla de login"""

    if 'autenticado' not in st.session_state:
        st.session_state.autenticado = False
        st.session_state.usuario = None

    # ── Sidebar (solo cuando ya está autenticado) ──────────────────────────
    with st.sidebar:
    ...
    # ── Leer credenciales enviadas via query_params ───────────────────────
    ...
    if _u and _p:
        ...
        if _u in passwords and hmac.compare_digest(_p, passwords[_u]):
    ...
    # ── Pantalla de login: HTML puro, cero widgets Streamlit ──────────────
    error_html = """
        <div style="background:#fee2e2;border:1px solid #fca5a5;border-radius:10px;
                    padding:10px 16px;color:#b91c1c;font-size:0.9rem;margin-bottom:12px;">
            ❌ Usuario o contraseña incorrectos
        </div>""" if _login_error else ""
    ...
    <style>
        ...
        /* Ocultar el contenedor blanco vacío que genera Streamlit */
        [data-testid="stVerticalBlock"] > div:empty,
        [data-testid="element-container"] {{ background: transparent !important; box-shadow: none !important; }}
    </style>
    ...
    ">
        <div style="
            ...
            box-shadow: 0 12px 48px rgba(11,61,145,0.13);
            width: 100%;
            max-width: 380px;
            text-align: center;
        ">
            <h1 style="color:#0b3d91;font-size:1.55rem;font-weight:700;margin:0 0 6px 0;">
                METAR Digital · SPJC
            </h1>
            <p style="color:#666;font-size:0.9rem;margin:0 0 28px 0;">Solo personal autorizado</p>

            {error_html}

            <form method="get" action="" autocomplete="on">
                <input name="_u" type="text" placeholder="Usuario" autocomplete="username"
                    style="
                        width: 100%; box-sizing: border-box;
                        border: 2px solid #e0e7ef; border-radius: 12px;
                        padding: 13px 16px; font-size: 0.95rem;
                        margin-bottom: 12px; outline: none;
                        transition: border-color .2s;
                    "
                    onfocus="this.style.borderColor='#0b3d91'"
                    onblur="this.style.borderColor='#e0e7ef'"
                />

                <div style="position:relative; margin-bottom: 20px;">
                    <input id="pwd" name="_p" type="password" placeholder="Contraseña" autocomplete="current-password"
                        style="
                            width: 100%; box-sizing: border-box;
                            border: 2px solid #e0e7ef; border-radius: 12px;
                            padding: 13px 48px 13px 16px; font-size: 0.95rem;
                            outline: none; transition: border-color .2s;
                        "
                        onfocus="this.style.borderColor='#0b3d91'"
                        onblur="this.style.borderColor='#e0e7ef'"
                    />
                    <span onclick="var i=document.getElementById('pwd');i.type=i.type=='password'?'text':'password'"
                        style="position:absolute;right:14px;top:50%;transform:translateY(-50%);
                               cursor:pointer;font-size:1.1rem;user-select:none;">👁</span>
                </div>
                ...
                <button type="submit"
                    style="
                        ...
                        padding: 13px 40px; font-size: 1rem; font-weight: 600;
                        cursor: pointer; transition: all .25s;
                        box-shadow: 0 4px 14px rgba(11,61,145,0.25);
                    "
                    onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 20px rgba(11,61,145,0.35)'"
                    onmouseout="this.style.transform='';this.style.boxShadow='0 4px 14px rgba(11,61,145,0.25)'"
                >
                    🔐 INGRESAR
                </button>
            </form>
