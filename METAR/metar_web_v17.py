...
def verificar_autenticacion():
    ...
    # ── Formulario HTML puro via components ───────────────────────────────
    error_bloque = (
        '<div style="background:#fee2e2;border:1px solid #fca5a5;border-radius:10px;'
        'padding:10px 16px;color:#b91c1c;font-size:0.9rem;margin-bottom:14px;">'
        '&#10060; Usuario o contraseña incorrectos</div>'
    ) if _login_error else ""
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h1 { color: #0b3d91; font-size: 1.5rem; font-weight: 700; margin-bottom: 6px; }
  p  { color: #666; font-size: 0.9rem; margin-bottom: 28px; }
  input {
    width: 100%; border: 2px solid #e0e7ef; border-radius: 12px;
    padding: 13px 16px; font-size: 0.95rem; outline: none;
  ...
  .pwd-wrap input { margin-bottom: 0; padding-right: 46px; }
  .eye {
...
<body>
<div class="card">
  <h1>METAR Digital &middot; SPJC</h1>
  <p>Solo personal autorizado</p>
  """ + error_bloque + """
  <form method="get" action="" autocomplete="on">
    <input name="_u" type="text" placeholder="Usuario" autocomplete="username" />
    <div class="pwd-wrap">
      <input id="pwd" name="_p" type="password" placeholder="Contrase&ntilde;a" autocomplete="current-password" />
      <span class="eye" onclick="var i=document.getElementById('pwd');i.type=i.type=='password'?'text':'password'">&#128065;</span>
    </div>
...
"""
    components.html(login_html, height=420, scrolling=False)
    st.stop()
