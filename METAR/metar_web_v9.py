...
with col_header1:
    st.markdown("<h1 style='color: #0b3d91;'>METAR DIGITAL - SPJC</h1>", unsafe_allow_html=True)
...
with col_der:
    st.markdown("<div class='section-title'>METAR GENERADO</div>", unsafe_allow_html=True)
    if 'ultimo_metar' in st.session_state:
        tipo_ultimo = st.session_state.get('ultimo_tipo', 'METAR')
        if tipo_ultimo == "SPECI":
            st.markdown(f"<div style='background: #FFE699; padding: 15px; border-radius: 5px; font-family: monospace; border-left: 5px solid #FFC000;'><b>SPECI</b><br>{st.session_state.ultimo_metar}</div>", unsafe_allow_html=True)
        else:
