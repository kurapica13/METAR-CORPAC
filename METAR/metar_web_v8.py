...
Características:
✅ Sin reloj - El operador ingresa hora manualmente
...
def limpiar_campos():
    ...
    st.session_state.hora = ""  # HORA VACÍA - El operador debe ingresarla
...
if not st.session_state.campos_inicializados:
    st.session_state.dia = datetime.now(timezone.utc).strftime("%d")
    st.session_state.hora = ""  # HORA VACÍA
    st.session_state.tipo = "METAR"
    st.session_state.dir_viento = ""
    st.session_state.int_viento = ""
    st.session_state.var_viento = ""
...
def exportar_a_excel(registros):
    """Genera un archivo Excel para descargar"""
    if not registros:
...
with col_header2:
    ...
    <div style='text-align: right;'>
        ...
        <p style='color: #999; font-size: 11px;'>CORPAC PERÚ</p>
    </div>
    """, unsafe_allow_html=True)
...
with col_izq:
    with st.form(key='metar_form'):
        ...
        with col2:
            dia = st.text_input("Día", key='dia', help="Formato: DD (01-31)")
        with col3:
            hora = st.text_input("Hora UTC", key='hora', help="⚠️ INGRESE HORA MANUALMENTE - Formato HHMM (ej: 1230)")
        
        st.markdown("<p style='color: #ff0000; font-size: 12px;'>⚠️ La hora UTC es OBLIGATORIA - Debe ingresarla manualmente</p>", unsafe_allow_html=True)
...
<div style='text-align: center; color: #666; padding: 20px;'>
    ...
    <p style='font-size: 0.8rem;'>⚠️ La hora UTC debe ser ingresada manualmente por el operador</p>
</div>
""", unsafe_allow_html=True)
