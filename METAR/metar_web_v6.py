...
def cargar_registros_mes():
    ...
    if archivo.exists():
        try:
            ...
            # Convertir a lista de diccionarios y normalizar claves
            registros = []
            for _, row in df.iterrows():
                registro = row.to_dict()
                
                # Normalizar claves para mantener consistencia
                if 'DIA' in registro:
...
def actualizar_o_insertar_registro(registros, nuevo_registro):
    ...
    """
    # Obtener día y hora del nuevo registro (manejar ambas claves posibles)
    dia_nuevo = nuevo_registro.get('Día', nuevo_registro.get('Dia', ''))
    hora_nueva = nuevo_registro.get('Hora', '')
    
    # Asegurar formato de 2 y 4 dígitos
    dia_nuevo = str(dia_nuevo).zfill(2)
    hora_nueva = str(hora_nueva).zfill(4)
    ...
    for i, registro in enumerate(registros):
        dia_existente = registro.get('Día', registro.get('Dia', ''))
        hora_existente = registro.get('Hora', '')
        
        dia_existente = str(dia_existente).zfill(2)
        hora_existente = str(hora_existente).zfill(4)
    ...
    # Guardar automáticamente en el archivo mensual
...
def generar_metar(datos):
    try:
        ...
        registro = {
            'Día': str(datos['dia']).zfill(2),  # Siempre con tilde y formato 2 dígitos
            'Hora': hora,  # Siempre con 4 dígitos
            'Tipo': datos['tipo'],
...
with col_header2:
    ...
    archivo_actual = obtener_nombre_archivo_mensual()
    st.markdown(f"<p style='color: #0b3d91; text-align: right; font-size: 12px;'>📁 {archivo_actual}</p>", unsafe_allow_html=True)
...
with col_izq:
    with st.form(key='metar_form'):
        ...
        with col2:
            dia = st.text_input("Día", key='dia', help="Formato: DD")
        with col3:
        ...
        with col2:
            vis_min = st.text_input("Visibilidad Mínima", key='vis_min', help="Ej: 1200SW, 0800NE, 1500SE, 2000NW")
        with col3:
        ...
        with col5:
            presion = st.text_input("Presión Estación", key='presion', help="Para el registro")
        ...
        if generar:
            ...
            if resultado['success']:
                ...
                st.session_state.historial = nuevo_historial[:20]  # Mantener últimos 20
                st.session_state.contador = len(st.session_state.registros)
                
                if accion == "actualizado":
                    st.warning("🔄 METAR ACTUALIZADO - Reemplazó reporte existente con la misma fecha/hora")
                else:
                    st.success("✅ METAR generado y guardado correctamente")
...
with col_der:
        ...
        if st.button("📥 Descargar Excel", use_container_width=True):
            if st.session_state.registros:
                ...
                if archivo.exists():
                    ...
                    st.warning("⚠️ No hay archivo para descargar")
            else:
    ...
    with col2:
        if st.button("💾 Guardar Ahora", use_container_width=True):
            if st.session_state.registros:
                success, mensaje = guardar_registros_mes(st.session_state.registros)
                if success:
                    st.success(mensaje)
                else:
                    st.error(mensaje)
    ...
    if st.button("🔄 Cargar Mes Actual", use_container_width=True):
        registros_cargados = cargar_registros_mes()
        st.session_state.registros = registros_cargados
        st.session_state.contador = len(registros_cargados)
        st.success(f"✅ Cargados {len(registros_cargados)} registros del mes actual")
        st.rerun()
    ...
    if st.session_state.registros:
        ...
        if 'Día' in df_resumen.columns:
            ...
            for _, row in resumen_dias.iterrows():
                st.markdown(f"**Día {str(row['Día']).zfill(2)}:** {row['Cantidad']} reportes")
...
<div style='text-align: center; color: #666; padding: 20px;'>
    ...
    <p style='font-size: 0.8rem;'>Almacenamiento Mensual Automático · Reemplazo automático por día/hora · Persistencia de datos</p>
</div>
