...
def verificar_autenticacion():
    ...
    <style>
        ...
        .login-title { color: #0b3d91; font-size: 1.2rem; font-weight: 700; margin: 0 0 2px 0; }
        .login-sub  { color: #888; font-size: 0.78rem; margin: 0 0 18px 0; }
    </style>
    ...
    with col_c:
        ...
        with st.form("login_form"):
            ...
            contraseña = st.text_input("Contraseña", type="password", placeholder="contraseña")
...
def interpretar_nubes(texto, vis_m, fenomeno):
    ...
    tipos_nubes = {"CU","SC","ST","CB","TCU","AC","AS","NS","CI"}
    capas = texto.split(",")
    codigos_nubes = []
...
with col_izq:
    with st.form(key='metar_form'):
        ...
        FENOMENOS_OPCIONES = {
            ...
            "Polvo/Arena (DU/SA)": "DU",
            "Humo (FU)": "FU",
        }
        opciones_keys = list(FENOMENOS_OPCIONES.keys())
        ...
        if st.session_state.fenomenos_lista:
            tags_html = "".join(f"<span class='fenomeno-tag'>{f}</span>" for f in st.session_state.fenomenos_lista)
            st.markdown(f"<div style='margin:4px 0 8px 0'>{tags_html}</div>", unsafe_allow_html=True)
            fx_a_quitar = st.selectbox(
                "Quitar fenómeno",
                ["─ seleccionar ─"] + st.session_state.fenomenos_lista,
                key='fx_quitar',
                label_visibility="collapsed"
            )
            if st.form_submit_button("🗑 Quitar seleccionado", use_container_width=False):
                if fx_a_quitar in st.session_state.fenomenos_lista:
                    st.session_state.fenomenos_lista.remove(fx_a_quitar)
        else:
        ...
        TIPOS_NUBE = ["ST", "SC", "CU", "TCU", "CB", "AC", "AS", "NS", "CI"]
        OCTAS_LABELS = {
        ...
        if generar:
            # Construir texto de fenómenos desde lista
            fenomeno_texto = " ".join(st.session_state.fenomenos_lista)
            ...
            datos = {
                ...
                'fenomeno': fenomeno_texto,
                'nubes': st.session_state.nubes_lista,  # lista estructurada
            ...
            if resultado['success']:
                # Texto descriptivo de nubes para guardar en registro
                ...
                resultado['registro']['Fenómeno_Texto'] = fenomeno_texto
