...
def verificar_autenticacion():
    ...
    if submit:
        ...
        # Si no hay secrets configurados, usar credenciales por defecto (solo para pruebas)
        try:
            passwords = st.secrets.get("passwords", {})
            if not passwords:
                # Credenciales por defecto solo para pruebas iniciales
...
def redondear_metar(valor):
    ...
    try:
        ...
        return int(round(float(valor)))  # Fallback seguro
...
def exportar_a_excel(registros):
    ...
    try:
        ...
        # ===== CORRECCIÓN: Manejar valores nulos =====
        df = df.fillna('')
        ...
        # ===== CORRECCIÓN: Formatear DÍA y HORA con .str solo después de asegurar que son string =====
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # ===== CORRECCIÓN: Solo aplicar formato si hay columnas =====
...
with col_izq:
    with st.form(key='metar_form'):
        ...
        with col3:
            hora = st.text_input("Hora UTC", key='hora', help="INGRESE HORA MANUALMENTE - Formato HHMM")
        ...
        fenomeno = st.text_input("Fenomeno", key='fenomeno', help="Ej: niebla parcial, lluvia moderada, tormenta")
...
<div style='text-align: center; color: #666; padding: 20px;'>
    ...
    <p style='font-size: 0.8rem;'>Sistema de Registro de Observaciones</p>
</div>
""", unsafe_allow_html=True)
